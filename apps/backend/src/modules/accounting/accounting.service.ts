...
export async function getProfitAndLoss(userId: string, from: string, to: string) {
  ...
  // 1. Revenue
  const [salesIncome, otherIncome, jvIncome] = await Promise.all([
    ...
    prisma.journalItem.aggregate({
      where: {
        account: { shop_id: shop.id, group: { type: 'income' } },
        type: 'credit',
        journal: { entry_date: dateFilter }
      },
      _sum: { amount: true }
    })
  ]);

  const totalRevenue = Number(salesIncome._sum.amount ?? 0) + Number(otherIncome._sum.amount ?? 0) + Number(jvIncome._sum.amount ?? 0);

  // 2. COGS (Direct Expenses)
  const [allPurchasesInRange, allReturnsInRange, jvDirectExp] = await Promise.all([
    prisma.purchaseEntry.aggregate({
    ...
    prisma.journalItem.aggregate({
      where: {
        account: { shop_id: shop.id, group: { name: 'Direct Expenses' } },
        type: 'debit',
        journal: { entry_date: dateFilter }
      },
      _sum: { amount: true }
  ...
  const cogs = costOfPurchases - costOfReturns + Number(jvDirectExp._sum.amount ?? 0);
  const grossProfit = totalRevenue - cogs;

  // 3. Indirect Expenses
  const [allExpenses, jvIndirectExp] = await Promise.all([
    ...
    prisma.journalItem.groupBy({
      by: ['account_id'],
      where: {
        account: { shop_id: shop.id, group: { type: 'expense', name: { not: 'Direct Expenses' } } },
        type: 'debit',
        journal: { entry_date: dateFilter }
      },
      _sum: { amount: true }
  ...
  // Add Journal Expenses
  for (const jv of jvIndirectExp) {
    const acc = await prisma.chartOfAccount.findUnique({ where: { id: jv.account_id }, select: { name: true } });
    const cat = acc?.name || 'Other Expense';
    const amt = Number(jv._sum.amount ?? 0);
  ...
  return {
    ...
    revenue: {
      ...
      other_income: Number(otherIncome._sum.amount ?? 0) + Number(jvIncome._sum.amount ?? 0),
      total: totalRevenue,
    },
...
export async function getCashbook(userId: string, opts: { from: string; to: string }) {
  ...
  const [income, expenses, supplierPay, saleRets, contras, jvItems] = await Promise.all([
    ...
    prisma.journalItem.findMany({
      where: {
        account: { shop_id: shop.id, name: 'Cash in Hand' },
        journal: { entry_date: dateFilter }
      },
      include: { journal: true },
      orderBy: { journal: { entry_date: 'asc' } }
    })
  ]);
  ...
  for (const jv of jvItems) {
    lines.push({
      date: jv.journal.entry_date.toISOString().slice(0, 10),
      type: 'journal',
      narration: jv.journal.description || 'Journal Entry',
      // Convention in this file: credit = In (debit in COA), debit = Out (credit in COA)
      debit: jv.type === 'credit' ? Number(jv.amount) : 0,
      credit: jv.type === 'debit' ? Number(jv.amount) : 0,
      method: 'cash'
    });
  }
...
export async function getBankbook(userId: string, opts: { from: string; to: string; method?: string }) {
  const shop = await getShopOrThrow(userId);
  const dateFilter = { gte: new Date(opts.from), lte: new Date(opts.to) };
  const bankMethods: any[] = opts.method ? [opts.method] : ['upi', 'neft', 'cheque', 'card', 'bank_transfer'];
  const [income, expenses, supplierPay, saleRets, contras, jvItems] = await Promise.all([
    ...
    prisma.journalItem.findMany({
      where: {
        account: { shop_id: shop.id, name: 'Bank Account' },
        journal: { entry_date: dateFilter }
      },
      include: { journal: true },
      orderBy: { journal: { entry_date: 'asc' } }
...
async function calculateBalanceBefore(shopId: string, date: Date, methods: string[]) {
  ...
  if (methods.includes('cash')) {
    baseBalance += Number(shop.opening_cash_balance);
    accountNames.push('Cash in Hand');
  }
  if (methods.some(m => ['upi', 'neft', 'cheque', 'card'].includes(m))) {
    baseBalance += Number(shop.opening_bank_balance);
    accountNames.push('Bank Account');
  }
  ...
  const jvNet = Number(jvDebits._sum.amount || 0) - Number(jvCredits._sum.amount || 0);
  ...
  return baseBalance + (Number(inc._sum.amount) || 0) - (Number(exp._sum.amount) || 0) - (Number(sup._sum.amount) || 0) - (Number(sret._sum.total_amount) || 0) + contraSum + jvNet;
}
...
export async function generateGstr2Excel(userId: string, month: number, year: number) {
  ...
  const b2bSheet = workbook.addWorksheet('b2b');
  b2bSheet.columns = [
    { header: 'GSTIN of Supplier', width: 20 },
    { header: 'Invoice Number', width: 15 },
    { header: 'Invoice date', width: 15 },
    { header: 'Invoice Value', width: 15 },
    { header: 'Place Of Supply', width: 20 },
    ...
    { header: 'Rate', width: 10 },
    { header: 'Taxable Value', width: 15 },
    { header: 'Integrated Tax Paid', width: 15 },
    { header: 'Central Tax Paid', width: 15 },
    { header: 'State/UT Tax Paid', width: 15 },
    { header: 'Cess Paid', width: 12 },
    { header: 'Eligibility For ITC', width: 20 },
    { header: 'Availed ITC Integrated Tax', width: 20 },
    { header: 'Availed ITC Central Tax', width: 18 },
    { header: 'Availed ITC State/UT Tax', width: 20 },
    { header: 'Availed ITC Cess', width: 15 },
  ];

  // 2. B2BUR Sheet
  const b2burSheet = workbook.addWorksheet('b2bur');
  b2burSheet.columns = [
    { header: 'Supplier Name', width: 25 },
    { header: 'Invoice Number', width: 15 },
    { header: 'Invoice date', width: 15 },
    { header: 'Invoice Value', width: 15 },
    { header: 'Place Of Supply', width: 20 },
    { header: 'Supply Type', width: 15 },
    { header: 'Rate', width: 10 },
    { header: 'Taxable Value', width: 15 },
    { header: 'Integrated Tax Paid', width: 15 },
    { header: 'Central Tax Paid', width: 15 },
  ...
  cdnrSheet.columns = [
    { header: 'GSTIN of Supplier', width: 20 },
    { header: 'Note/Refund Voucher Number', width: 20 },
    { header: 'Note/Refund Voucher Date', width: 15 },
    { header: 'Note/Refund Voucher Value', width: 15 },
    { header: 'Place Of Supply', width: 20 },
    { header: 'Note Type', width: 10 },
    { header: 'Reverse Charge', width: 15 },
    { header: 'Rate', width: 10 },
  ...
  // Add dummy sheets for template compliance
  workbook.addWorksheet('at');
  workbook.addWorksheet('atadj');
  workbook.addWorksheet('exemp');
  workbook.addWorksheet('itcfel');

  // 5. HSN Summary Sheet
  const hsnSheet = workbook.addWorksheet('hsnsum');
  hsnSheet.columns = [
    { header: 'HSN', width: 15 },
    { header: 'Description', width: 25 },
    { header: 'UQC', width: 10 },
  ...
  for (const pur of purchases) {
    ...
    for (const rate of rates) {
      ...
      if (isRegistered) {
        b2bSheet.addRow([
          pur.supplier?.gst_number, pur.invoice_number, dateStr, Number(pur.total_amount), pos, 'N', 'Regular', rate,
          taxableAtRate, isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2,
          0, 'Inputs', isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2, 0
        ]);
      } else {
        b2burSheet.addRow([
          pur.supplier?.name || 'Unregistered', pur.invoice_number, dateStr, Number(pur.total_amount), pos,
          isInterState ? 'Inter-State' : 'Intra-State', rate, taxableAtRate, isInterState ? gstAtRate : 0,
          isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2, 0, 'Inputs',
          isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2, 0
        ]);
      }
    }
  ...
  for (const ret of purchaseReturns) {
    ...
    for (const rate of rates) {
      ...
      if (isRegistered) {
        cdnrSheet.addRow([
          ret.supplier?.gst_number, ret.return_number, dateStr, Number(ret.total_amount), pos, 'D', 'N', rate,
          taxableAtRate, isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2,
          0, 'Inputs', isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2, 0
        ]);
      } else {
        cdnurSheet.addRow([
          ret.return_number, dateStr, Number(ret.total_amount), pos, 'D', rate, taxableAtRate,
          isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2,
          0, 'Inputs', isInterState ? gstAtRate : 0, isInterState ? 0 : gstAtRate / 2, isInterState ? 0 : gstAtRate / 2, 0
        ]);
      }
    }
...
export async function getBalanceSheet(userId: string, date: string) {
  ...
  const report = await Promise.all(groups.map(async (group) => {
    const accountsWithBalance = await Promise.all(group.accounts.map(async (acc) => {
      ...
      // Liability/Equity/Income: Normal Credit (Balance = Credits - Debits)
      let balance = 0;
      if (group.type === 'asset' || group.type === 'expense') {
        balance = Number(acc.opening_balance || 0) + debits - credits;
      } else {
        balance = Number(acc.opening_balance || 0) + credits - debits;
      }
    ...
    return {
      ...
      total: Math.round(accountsWithBalance.reduce((sum, a) => sum + a.balance, 0) * 100) / 100
    };
  }));
...
export async function generateGstr1Json(userId: string, month: number, year: number) {
  ...
  const bills = await prisma.bill.findMany({
    where: { shop_id: shop.id, created_at: { gte: start, lte: end }, payment_status: 'paid' },
  ...
  for (const bill of bills) {
    ...
    const pos = bill.billing_state || (shop.state_code || '27'); // Fallback to shop state
    
    // Grouping for HSN
    for (const item of bill.items) {
      const hsn = item.hsn_code || (item.inventory?.medicine as any)?.hsn_code || '3004'; 
      const txval = Number(item.line_total) / (1 + Number(item.gst_rate) / 100);
      const tax = Number(item.line_total) - txval;
      ...
      hEntry.val += Number(item.line_total);
      hEntry.txval += txval;
    ...
    if (isB2B) {
      let ctinEntry = b2b.find(b => b.ctin === bill.customer_gstin);
      if (!ctinEntry) {
        ctinEntry = { ctin: bill.customer_gstin, inv: [] };
        b2b.push(ctinEntry);
      }
      
      const invItems: any[] = [];
      const rates = Array.from(new Set(bill.items.map(i => Number(i.gst_rate))));
      rates.forEach((rt: number, idx) => {
        const rateItems = bill.items.filter(i => Number(i.gst_rate) === rt);
        const txvalTotal = rateItems.reduce((acc, i) => acc + (Number(i.line_total) / (1 + rt / 100)), 0);
        const taxTotal = rateItems.reduce((acc, i) => acc + (Number(i.line_total) - (Number(i.line_total) / (1 + rt / 100))), 0);
        
        invItems.push({
          num: idx + 1,
          itm_det: {
            rt,
            txval: Math.round(txvalTotal * 100) / 100,
            iamt: pos !== shop.state_code ? Math.round(taxTotal * 100) / 100 : 0,
            camt: pos === shop.state_code ? Math.round((taxTotal / 2) * 100) / 100 : 0,
            samt: pos === shop.state_code ? Math.round((taxTotal / 2) * 100) / 100 : 0,
          }
        });
      });
      ...
      ctinEntry.inv.push({
        ...
        idt: fd(bill.created_at),
        val: Math.round(Number(bill.total_amount) * 100) / 100,
        pos,
        rchrg: 'N',
        inv_typ: 'R',
        itms: invItems
      ...
      rates.forEach((rt: number) => {
        ...
        b2csEntry.txval += txvalTotal;
        if (pos !== shop.state_code) b2csEntry.iamt += taxTotal;
        else { b2csEntry.camt += taxTotal / 2; b2csEntry.samt += taxTotal / 2; }
      });
    }
  }
  ...
  return {
    gstin: shop.gst_number || 'NOT_FOUND',
